...
<span style="color: green">**Browse the pre-existing packages in your RStudio. Then try googling or looking up packages for your specific purposes (e.g., by typing “R package for [your purpose]”).**</span>
...
If the package you want to use is already present in RStudio, all you need to do is load it using the `library()` function. However, if the package is not installed, you have to do it and then load it with the `library()` function. 
...
<span style="color: green;">**Check the `read_table()` and the `read.csv()` function manuals. Upload the "class_2_data.csv" file into the R environment using both functions. Make sure what additional arguments you should use for both functions to upload files properly.**
...
<span style="color: darkorange">**Upload both your R script and .csv file to the "Class 2" tab on the *Pegaz* platform.**</span>
