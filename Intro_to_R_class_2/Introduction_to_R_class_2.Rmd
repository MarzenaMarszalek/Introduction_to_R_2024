...
author: " Mateusz Chechetkin, Tomasz Gaczorek, Wiesław Babik & Marzena Marszałek<br> marzena.marszalek@doctoral.uj.edu.pl<br>"
date: "`r Sys.Date()`"
...
![](rstudio.png)
  ...
  1. **Code editor** - Here you can write your code and save it in a text file (.R). You can run it anytime by highlighting the given piece of code and clicking `Run` in the top-right corner (also Ctrl + Enter).

  2. **Console** - The same console as in standard R (see Class 1). Note that any line of code run from the Code editor will appear in the console. To recall the last run line of code, use `Arrow-up` (it works as a general way of browsing through the history of the executed commands).
...
To run a particular line(s) from an R script, **highlight this part of your code** and press **Ctrl + Enter** or click the `Run` button. To run the whole script, click the `Source` button. The results will appear in the **Console**.

![](RScript.png)

### <span style="color: green">**Exercise 1**</span>
<span style="color: green;">**Create a new script file and save it. Name it as you wish. Write code for all subsequent exercises using the Code editor. Remember that to obtain any outcome/result, you need to run your code first.**</span>

To make your code easier to read and understand, it is a good practice to **add comments**. Comments help explain what specific sections of the code are doing. This may be useful for other people using your script, and for you as well. Even if you know what your code is doing, after some time, you may forget it, and you may have to spend a lot of time trying to understand its functionalisty again.
...
<span style="color: green;">**Create a new folder on your computer and set it as a working directory.**</span>
...
**Every time you boot up RStudio, remember to:**  
  -	 <span style='color: #265843ff'>**create and save a new script file**</span> – if you don’t have one already  
  -	<span style='color: #265843ff'>**copy and update the name of your script file (e.g. with the current date)**</span> – if you are working on a previously created file and want to **keep track of multiple versions** 
  -	<span style='color: #265843ff'>**set up a working directory** </span> – easiest way to keep track of this is to have a **line of code at the beginning of the script**; if you set up a working directory through the RStudio user interface, you can copy the code from the console into the script  
  -	<span style='color: #265843ff'>**clear the RStudio environment** </span> – this is important, especially when **switching between scripts/projects** you are working on  
  - <span style='color: #265843ff'>**load any packages that the script is using**</span> – further explanation of this will be in the section about packages; for now, remember that some functions we will use later on **are not present in base R**, so if RStudio suddenly doesn’t want to run code with a function that you’ve been using before, it might be because it comes from a package that **isn’t currently loaded**
...
<span style="color: green;">**Download the [class_2_data.xlsx](class_2_data.xlsx) file and save it in your working directory. Then, upload the file into R using the RStudio user interface.**</span>
...
<span style="color: green;">**Install the `ggplot2` package using the `install.packages()` function. Check the `Packages` tab - a newly installed package should appear there!**</span>

#### <span style="color: green;">**Exercise 5**</span>
<span style="color: green;">**Install the `tidyverse` package. Then, [see here](https://www.tidyverse.org/) to learn more about `tidyverse`. Load the `dplyr` package.**</span>
...
Uploading a .csv file into R **does not require any additional packages**. We can do it using a `read.csv()` or `read.table()` function.
...
These arguments are, for example:  
  ...
  - **encoding**: specifies which encoding is used in the .csv file; this can be important if you are working with files that contain non-Latin characters, for example Polish special letters (ą, ę, ć, etc.), but it isn’t always required.
...
<span style="color: green;">**Check the `read.table()` and the `read.csv()` function manuals. Upload the "class_2_data.csv" file into the R environment using both functions. Make sure what additional arguments you should use for both functions to upload files properly.**
...
Tips for finding R solutions on the Internet:
  ...
  - **AI-generated solutions** are very useful but require some care: while ChatGPT, Gemini and other similar tools often generate correct code, this is **not guaranteed**; **if the code uses functions you don’t know, you might not notice that it generates faulty results**

#### <span style="color: green;">**Exercise 8**</span>
<span style="color: green;">**Search the Internet for an R function that can rename columns in a data frame. Install and load a package if necessary. Try using the RStudio manual to figure out how to use the function that can perform this task. Use the `rename()` function to change one of the columns' names.**</span>

Remember that the **manual** for any installed function can be invoked up by typing a **question mark (?) before its name** or by **pressing F1 when your cursor is on the function name.**
If you are attempting to use someone else’s code and you need to adapt it for your purposes, consider the following:

  -	 before changing anything in the code you copied, go through the entire script and **make sure you understand**, at least on a basic level, what every line is doing. Again ChatGTP and others may be helpful here. 
...
You will likely encounter <span style="color: red;">**warning messages**</span> and <span style="color: red;">**error messages**</span> daily while using R. This does not mean you are careless or bad at coding – errors and warnings exist to **guide you in the process of using R**. Because computers are very particular about details and can be unintuitive for us, it takes effort to understand them. Knowing how to react to errors is a big component of an effective coding workflow.  
...
<span style="color: green">**Look at these lines of code. What kind of warning messages do you think they contain? Run the lines of code separately, then identify what warning is occurring and why. If you know how to fix the issue, edit the code.**</span>
...
Some common causes for errors in R can be:
  ...
  -	<span style="color: red">**Error in [code]: object not found**</span> - you are trying to operate on an **object that does not exist**; this can happen because your file did not load into R (e.g. because the working directory is set incorrectly), because of a typo in the object’s name (remember that names in R are case-sensitive!) or because the object was deleted from the environment  
...
In most of your analyses, you will not need to download a modified data frame out of R – you will only be interested in the results of certain tests or graphs that R will generate. However, if you want to **download the data frame**, you can use:  
  - `write.table()` – the base R function, for saving a .csv file; uses similar arguments to `read.table()`, uses blank space (` `) as delimiter by default  
  - `write.csv()` - the base R function, specialized version of `write.table()` for CSV files, uses comma (`,`) as delimiter by default  
  - `write_xlsx()` – requires the `writexl` package to be installed and loaded

#### <span style="color: green;">**Exercise 12**</span>
<span style="color: green">**Using the `write.table()` function write the `class_2_data` table as a .csv file.**</span>
...
As you can see, coding can be meticulous and sometimes frustrating work. To reduce this frustration, remember the following:
  ...
  -	if you are working on a script for multiple days, consider making **a new copy of it every day or every week** to keep previous versions in hand in case you need to revert the changes (there are much better ways of handling thee problem of version controm but they are beyond the scope of our course)
...
<span style="color: #da205eff">
  ...
  2.	Upload the [homework_data.xlsx](homework_data.xlsx) file into RStudio using your preferred method. **Include the code for uploading the file into the script**. Remember that some upload methods require packages to be loaded first.<br>
  3.	Load the `dplyr` package. **Include the loading of the package as a line of code in your script**.<br>
  4.	Find a `dplyr` function that can **sort a data frame based on the values of the given column** of a data frame. Use it to sort the homework data frame according to the values in the column `score` from **lowest to highest**.<br>
  5.	Save the result of exercise 4 as a .csv file using `write.table()` function. Call the .csv file “class_2_sorted_column_Your_Name.csv”. Check the `write.table()` function manual to find out **what arguments you should use** to save a .csv file properly. The file should have column names and fields should be separated by commas `,`.

<span style="color: #da205eff">**Upload your R script file to the "Class 2" tab on the *Pegaz* platform.**</span>
