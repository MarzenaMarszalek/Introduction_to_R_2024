...
<span style="color: green">**Using function from `dplyr` package obtain the observations from `my_data` sorted firstly by `Treatment` column and then by `Rat` column. Both in ascending order.**</span>
...
<span style="color: green">**Obtain the observations for `Treatment` equals 1.**</span>
...
<span style="color: green">**Obtain the observations for `Treatment` equals 3 with glycogen level higher than 135.**</span>
...
<span style="color: green">**Using pipeline and `my_data`: 1. select `ID`, `Glycogen` and `Liver` column. 2. obtain observations with glycogen level lower than 140. 3. sort it based on `Glycogen` column in descending order.**</span>
...
```{r}
v31 <- group_by(my_data, Treatment) %>%  mutate(std_dev = sd(Glycogen))
v31[1:10,]
```
**************** **ADVANCED** ******************  
...
4.	Using `dplyr` function add a new column to the `hw_data` data frame that displays the `uptake` value as a percent of the maximum value of that column (=so that the highest value, 45.5, corresponds to 100%). Then remove the original `uptake` column using a `dplyr` function.<br>
5.	Use the pipe operator to perform multiple functions on the `hw_data` data frame. Create a summary data frame that displays mean and standard deviation of the new `uptake` column for `Treatment` groups. Save this summary as a .csv file called "CO2_uptake_summary_Your_Name.csv".
