...
It’d also be nice to to show each species with different colour.

#### <span style="color: green">**Exercise 16**</span> 
<span style="color: green">**Generate a histogram of sepal length. Set bars' color according to `Species`.**</span>

Expected result:

```{r}
ggplot(my_data) + geom_histogram(aes(x = Sepal.Length, fill = Species), color = "black") +
  theme_bw() +
...
```{r, message = F}
ggplot(my_data) +
  geom_histogram(aes(x = Sepal.Length, y = after_stat(count)), fill =  "white", color = "black") +
  theme_bw() +
  labs(x  = "Length of sepal [cm]", y = "Frequency", title = "Histogram of sepals' length observation") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
```{r, message = F}
ggplot(my_data) + geom_histogram(aes(x = Sepal.Length, y = after_stat(density)), fill = "white", color = "black") +
...
```{r, message = F}
ggplot(my_data) + 
  geom_histogram(aes(x = Sepal.Length, y = after_stat(density)), fill = "white", color = "black", bins = 30) + 
