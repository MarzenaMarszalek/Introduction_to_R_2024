...
## <span style="color: #3b50dbff">**Basic plotting @home**</span>
...
### <span style="color: #3b50dbff">**Exercise 1 @home**</span>
<span style="color: #3b50dbff">**Assign the internal data set `iris` to an object called `my_data`. View the dataset and create its short summary.**</span>
...
The two most common types of plots are **scatterplots** and **histograms**.
  ...
  - **Histograms**, on the other hand, display number of observations (shown on the y axis of a graph) per category of observations (shown on the x axis). Histograms are usually used to demonstrate differences between groups or their central tendencies (e.g. means).

### Scatterplot 

We’ll start with a scatterplot. The basic R function to create any kind of plot is, simply, `plot()`, it automatically adapts depending on the type of object you pass to it (numeric vectors, data frames, factors, etc.). To set it up, provide arguments for x axis, and y axis: `plot(x = x_axis_data, y = y_axis_data)`, where `x` and `y` specify variables that define x and y axes values respectively. 

#### <span style="color: #3b50dbff">**Exercise 2 @home**</span> 
<span style="color: #3b50dbff">**Create a  scatterplot using `my_data` data frame. Provide the `Sepal.length` column as x, and `Sepal.width` as y arguments of the `plot()` function.**</span>
...
#### <span style="color: #3b50dbff">**Exercise 3 @home**</span> 
<span style="color: #3b50dbff">**Create a plot using the same function but writing the arguments down as `Sepal.width` depending on `Sepal.length`. Provide the name of the data frame as data argument. Remember that, on a standard plot, the y axis shows the dependent variable, and x shows the independent variable.**</span>
...
#### <span style="color: #3b50dbff">**Exercise 4 @home**</span> 
<span style="color: #3b50dbff">**Copy and modify the code from exercise 3. Add axis titles using the arguments `xlab` and `ylab`. Add the title of the plot using the `main` argument.**</span>
...
<span style="color: #3b50dbff">**Modify the plot function to color the observations by `Species`. Do this by adding a `col` argument and setting the `Species` column as its value.**</span>
...
Now let’s try to make a histogram. The base R function for making a histogram is `hist()`. The only argument you need to provide is the **column of the data you want to work on**. The group of the observations will be shown on the x axis, and the number of observations in that group will be shown on the y axis.

#### <span style="color: #3b50dbff">**Exercise 6 @home**</span> 
<span style="color: #3b50dbff">**Create a histogram plot of the sepal length using `my_data`.**</span>
...
#### <span style="color: #3b50dbff">**Exercise 7 @home**</span> 
<span style="color: #3b50dbff">**Update the histogram code by adding axes names, title, and setting it to display 4 bins.**</span>
...
From now on, we are going to use the `ggplot2` package that provides a flexible system for building complex and visually appealing graphics in R. [Here](https://r-graph-gallery.com/) you can find very nicely presented descriptions of many types of plots.
...
<span style="color: green">**Modify the histogram above and display both density distributions and density bars for each species separately on a single plot. Take advantage of `color` argument inside aesthetics.**</span>

Expected result:

```{r, message = F}
ggplot(my_data, aes(x = Sepal.Length, y = after_stat(density))) + 
  geom_histogram(aes(fill = Species), color = "black", bins = 10, position = "dodge") + 
...
We can also display **separate plots for each species**, but still keep them within a **single figure**, using the `facet_wrap()` function. Add `facet_wrap(~ variable)` to your ggplot chain, where **variable is the factor you want to split the plot by**. Panels are automatically arranged in a grid, making comparisons across groups easy. Since each species is in its own panel, there is no need to use `position = "dodge"` anymore.

#### <span style="color: green">**Exercise 22**</span> 
<span style="color: green">**Now modify the histogram above to display density distributions and density bars for each species on a separate plots.**</span>
...
So far, when we used the `color` or `fill` arguments inside `aes()`, `ggplot2` **automatically selected colors**. You can also **set colors manually** by specifying exact values using `scale_fill_manual()` or `scale_color_manual()`. For example:
`scale_fill_manual(values = c(group1 = "color1", group2 = "color2", ...))` or `scale_fill_manual(values = c("color1", "color2", ...))` 

You can also use predefined **color palettes** from packages like `RColorBrewer` or `viridis` to **automatically generate visually appealing and consistent colors**. Palettes are especially useful when working with many categories, as they ensure good contrast and accessibility. [Here](https://www.datanovia.com/en/blog/top-r-color-palettes-to-know-for-great-data-visualization/) you can read more about color palettes and how to apply them.

#### <span style="color: green">**Exercise 23**</span> 
<span style="color: green">**Change the colors for each species in the plot you created in the previous exercise.**</span>
...
ggplot(my_data, aes(x = Sepal.Length, y = after_stat(density))) + 
  ...
  theme(plot.title = element_text(hjust = 0.5)) +
  facet_wrap(~ Species) +
  scale_fill_manual(values = c(setosa = "violet", virginica = "lightblue", versicolor = "pink"))
```
...
R gives you the ability to generate reports in a variety of formats, including HTML, PDF, and Word. This is done using the R Markdown package. R Markdown allows you to combine text, code, and output (including plots) into a single document, generating a report based on the code and text in your script. 

The basic method to create a report is to create such report straight from R script files. You can do this using the user interface by clicking on the `Compile report` button (see below) in RStudio or using the `rmarkdown::render()` function in the console. For more information, see [this guide](https://rmarkdown.rstudio.com/articles_report_from_r_script.html).

![](./knit.png)

A more flexible and widely used (though more advanced) approach is to create reports from R Markdown (.Rmd) files. Unlike standard R scripts, R Markdown files allow you to interleave formatted text with executable R code, enabling you to produce well-structured, fully reproducible reports. You can learn how to create and customize R Markdown documents in [the additional class instruction](https://marzenamarszalek.github.io/Introduction_to_R_2024/Rmarkdown_2025/Introduction_to_R_Rmarkdown.html).

#### <span style="color: green">**Exercise 25**</span> 
<span style="color: green">**Generate a Microsoft Word report containing the plots you created during today's class directly from your R Script file.**</span>

## <span style="color: #da205eff;">Homework</span>

<span style="color: #da205eff">
Use the built in `airquality` data set for all exercises. As a reminder, you can pull up a description of the inbuilt data set by typing a question mark (`?`) before its name into the Rconsole.
<br> 
  ...
  2. Using base R, generate a **histogram** for the measured **temperatures**. Set the **number of bins to 5**, add **axes descriptions**, and **plot title**.  
...
<span style="color: #da205eff">**Save all the graphs with your preferred method. Upload your R script (including commands for saving
