...
<span style="color: green">**View the uploaded data frame. Access `head()` function manual. View the top 10 rows of `my_data` data frame and generate a short description for it.**</span>
...
Data frame can be **subsetted** (=accessed or displayed in a specific way) with the use of **coordinates** (indexes) enclosed within the **square brackets** (`[]`). In case of data frames there are always 2 coordinates: `table[row_number, column_number]`.
...
You can call for **multiple values in the same time**. To define range of coordinates, use **colon** (`:`) separating range borders - e.g. `table[row_number_1:row_number_2, column_number]`.
...
<span style="color: green">**Return 4th column of `my_data` but use column name instead of its number. Don’t forget about quotation marks (`""`).**</span>
...
my_data$mass
```
Also, you can use simple summary functions from previous class (see Class 2).
...
All exercises need to be performed on the **built-in `ToothGrowth` dataset**. Remember you can find the dataset details checking its description with `?ToothGrowth`. To work on it, use this command:  
`my_data <- ToothGrowth`
Include all subsequent steps in a script file.<br>
  1.	Check the class of each column in `my_data` data frame. Find the the shortest tooth length.<br>
  ...
  3.	Add column with IDs - "obs_1" to "obs_n" ("n" is the number of the last observation)- for each observation as the first column of `my_data`. Overwrite `my_data` variable. Tip: to create a vector with IDs you can use `paste()` to connect "obs_" with the numbers (1-n) instead of typing each ID manually.<br>
  4.	Modify `my_data` by removing its 4 first rows. Overwrite `my_data` variable.<br>
...
print("Ex. 3")
ID <- paste("obs_", c(1:nrow(my_data)), sep = "")
my_data <- cbind(ID, my_data)
...
print("Ex. 5")
write.table(my_data, "homework.csv", sep = ",", row.names = FALSE)
