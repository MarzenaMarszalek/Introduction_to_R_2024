...
Include all subsequent steps in a script file.<br>
  ...
  3.	Add column with IDs - "obs_1" to "obs_n" ("n" is the number of the last observation)- for each observation as the first column of `my_data`. Overwrite `my_data` variable. Tip: to create a vector with IDs you can use `paste()` to connect "obs_" with the numbers (1-60) instead of typing each ID manually.<br>
